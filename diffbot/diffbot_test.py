	...
	def check(self):
		...
		for page in data:
			if page.get('type') == 'article':
				print page.get('author'),' | ',page.get('title'),' | ',page.get('timestamp'), " | ",page.get('pageUrl')
		return
##TESTING##
...
test_names_urls = [('nc_test','')
			 	  ...
			 	  ,('hartford_mile','http://extramile.thehartford.com')
				  ]

CRAWL_NUMBER = 1

name = test_names_urls[CRAWL_NUMBER][0]
url = test_names_urls[CRAWL_NUMBER][1]
crawl = Crawl(name,url)
